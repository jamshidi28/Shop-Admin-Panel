...
function CartItem({ cartItem }) {
  ...
  const { mutateAsync:removeCartAsync } = useRemoveCart()
...
const removeCartHandler =async ()=>{
  try {
  const {message}= await removeCartAsync(cartItem._id);
  toast.success(message)
  queryClient.invalidateQueries({queryKey:["get-user"]})
  } catch (error) {
    if(error?.response?.data){
    ...
    <div className="border rounded-xl p-4 flex justify-between">
      ...
      <div className="flex items-center justify-between  gap-x-8 flex-1">
        ...
        <div className="flex gap-x-3">
          ...
          <button onClick={removeCartHandler} className="border rounded p-1">
            {cartItem.quantity > 1 ? (
